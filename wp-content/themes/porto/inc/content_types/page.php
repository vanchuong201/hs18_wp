...
function porto_add_page_meta_boxes() {
    ...
    if ( function_exists('add_meta_box') && $screen && $screen->base == 'post' && $screen->id == 'page' ) {
        add_meta_box( 'page-meta-box', __('Page Options', 'porto'), 'porto_page_meta_box', 'page', 'normal', 'high' );
        add_meta_box( 'view-meta-box', __('View Options', 'porto'), 'porto_page_view_meta_box', 'page', 'normal', 'low' );
        if ($porto_settings['show-content-type-skin']) {
            add_meta_box( 'skin-meta-box', __('Skin Options', 'porto'), 'porto_page_skin_meta_box', 'page', 'normal', 'low' );
        }
    }
}
...
function porto_page_protected_meta($protected, $meta_key, $meta_type) {
    ...
    $screen = get_current_screen();
    if (!$protected && $screen && $screen->base == 'post' && $screen->id == 'page') {
