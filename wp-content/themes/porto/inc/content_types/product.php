...
if ( $attribute_taxonomies = wc_get_attribute_taxonomies() ) {
    foreach ( $attribute_taxonomies as $tax ) {
        if ( 'color' === $tax->attribute_type ) {
            add_action( wc_attribute_taxonomy_name( $tax->attribute_name ) . '_add_form_fields', 'porto_add_product_attribute_color_fields', 100, 2 );
            add_action( wc_attribute_taxonomy_name( $tax->attribute_name ) . '_edit_form_fields', 'porto_edit_product_attribute_color_fields', 100, 2 );
            add_action( 'edit_term', 'porto_save_product_attribute_color_values', 100, 3 );
            add_action( 'delete_term', 'porto_delete_product_attribute_color_values', 10, 5 );
        }
    }
}
...
function porto_add_product_attribute_color_fields( $tag, $taxonomy ) {
...
function porto_save_product_attribute_color_values( $term_id, $tt_id, $taxonomy ) {
    if ( strpos( $taxonomy, 'pa_' ) === false ) return;
    if ( $attribute_taxonomies = wc_get_attribute_taxonomies() ) {
        foreach ( $attribute_taxonomies as $tax ) {
            if ( 'color' === $tax->attribute_type && $taxonomy === wc_attribute_taxonomy_name( $tax->attribute_name ) ) {
                return porto_save_tax_meta_values( $term_id, $taxonomy, porto_get_product_attribute_color_fields(), true );
            }
        }
    }
}
function porto_delete_product_attribute_color_values( $term_id, $tt_id, $taxonomy, $deleted_term, $object_ids ) {
    if ( strpos( $taxonomy, 'pa_' ) === false ) return;
    if ( $attribute_taxonomies = wc_get_attribute_taxonomies() ) {
        foreach ( $attribute_taxonomies as $tax ) {
            if ( 'color' === $tax->attribute_type && $taxonomy === wc_attribute_taxonomy_name( $tax->attribute_name ) ) {
