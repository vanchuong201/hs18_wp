...
function porto_logo( $sticky_logo = false) {
    ...
    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?> - <?php bloginfo( 'description' ); ?>"<?php if (!$sticky_logo) echo ' rel="home"' ?>>
        <?php if ($porto_settings['logo-type']) : ?>
            <span class="logo-text"><?php echo $porto_settings['logo-text'] ? $porto_settings['logo-text'] : get_bloginfo('name', 'display'); ?></span>
        <?php else : ?>
        <?php if ($porto_settings['logo'] && $porto_settings['logo']['url']) {
            ...
            ?><span class="logo-text"><?php echo get_bloginfo('name', 'display'); ?></span><?php
        } ?>
        <?php endif; ?>
    </a>
...
function porto_banner($banner_class = '') {
                        ...
                        <div class="container-fluid">
                            ...
                            <span class="thumb-info-icons position-style-3 text-color-light">
                                <span class="thumb-info-icon pictures background-color-primary">
                                    <?php echo $portfolio_images_count; ?>
    ...
    <?php } else if ($banner_type === 'rev_slider' && isset($rev_slider) && class_exists( 'RevSlider' ) ) { ?>
        <div class="banner-container">
            <div id="banner-wrapper" class="<?php echo $banner_class ?>">
...
function porto_currency_switcher() {
	...
	if ( !$result && $porto_settings['wcml-switcher-html'] ) {
		$result = '<ul id="menu-currency-switcher" class="currency-switcher mega-menu show-arrow'. ($porto_settings['switcher-effect']?' '.$porto_settings['switcher-effect']:'') .'">
...
function porto_view_switcher() {
	...
	if ( !$result && $porto_settings['wpml-switcher-html'] ) {
		$result = '<ul id="menu-view-switcher" class="view-switcher mega-menu show-arrow'.($porto_settings['switcher-effect']?' '.$porto_settings['switcher-effect']:'').'">
                    <li class="menu-item has-sub narrow">
                        <h5><img src="'. porto_uri .'/images/flags/en.png" alt=""> English</h5>
						<div class="popup">
							<div class="inner">
								<ul class="sub-menu">
									<li class="menu-item"><a href="#"><img src="'. porto_uri .'/images/flags/en.png" alt=""> English</a></li>
									<li class="menu-item"><a href="#"><img src="'. porto_uri .'/images/flags/fr.png" alt=""> French</a></li>
								</ul>
							</div>
						</div>
                    </li>
...
function porto_mobile_view_switcher() {
    ...
    $result = str_replace('&nbsp;', '', ob_get_clean());
    if ( !$result && $porto_settings['wpml-switcher-html'] ) {
        $result = '<ul id="menu-view-switcher" class="view-switcher accordion-menu show-arrow">
                    <li class="menu-item has-sub narrow">
                        <h5><img src="'. porto_uri .'/images/flags/en.png" alt=""> English</h5>
                        <span class="arrow"></span>
                        <ul class="sub-menu">
...
function porto_minicart() {
    ...
    if ( class_exists( 'WooCommerce' ) ) :
        ?>
        <div id="mini-cart" class="mini-cart <?php echo $minicart_type ?><?php echo ($porto_settings['minicart-effect']?' '.$porto_settings['minicart-effect']:'') ?><?php echo ( $porto_settings['minicart-popup-style'] == '2' ) ? ' minicart-style2' : ' minicart-style1'; ?>">
            <div class="cart-head <?php echo str_replace('minicart-icon', 'cart-head', $porto_settings['minicart-icon']) ?>">
...
function porto_nofollow_block() {
    ...
    if ( is_singular() && isset( $post->post_type ) && $post->post_type === 'block' ) {
