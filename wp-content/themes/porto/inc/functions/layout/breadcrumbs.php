...
function porto_breadcrumbs_link($title, $link = '') {
	...
	$microdata_itemscope = ( ! is_front_page() && $title == 'Home' ) ? 'class="home"' : '';
    if ( $microdata ) {
        $microdata_itemscope .= 'itemscope itemtype="http://schema.org/BreadcrumbList"';
    ...
    $delimiter = '';
    if ( $link ) {
        $output = sprintf( '<a %s href="%s"%s>%s</a>', $microdata_url, $link, ( $title == 'Home' ) ? ' title="'.__('Go to Home Page').'"' : '', $output );
