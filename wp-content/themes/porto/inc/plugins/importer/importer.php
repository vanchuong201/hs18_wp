...
function porto_import_dummy() {
    ...
    if ( current_user_can( 'manage_options' ) && class_exists( 'WP_Importer' ) && class_exists( 'Porto_WP_Import' ) ) { // check for main import class and wp import class
        ...
        if ($response['process'] == 'complete' && $demo != 'shortcodes') {
            ...
            // Set reading options
            $homepage = porto_importer_get_page_by_title( 'Home' );
            if ($demo == 'law-firm' || $demo == 'finance') {
                $posts_page = porto_importer_get_page_by_title( 'News' );
            } else if ($demo == 'restaurant') {
                $posts_page = porto_importer_get_page_by_title( 'Press' );
            } else if ($demo == 'wedding') {
                $posts_page = porto_importer_get_page_by_title( 'Our Blog' );
            } else {
                $posts_page = porto_importer_get_page_by_title( 'Blog' );
            }
            ...
            // Set one page menu
            $onepage = porto_importer_get_page_by_title( 'Home One Page' );
            ...
            if ( class_exists( 'RevSlider' ) ) {
                ...
                if ( isset( $demos[$demo]['revslider'] ) && !empty( $demos[$demo]['revslider'] ) ) {
                    ...
                    foreach( $demos[$demo]['revslider'] as $rev ) {
                        $demo_path = $importer_api->get_remote_demo('revslider', $rev);
                        $slider_array[] = $demo_path;
                    }
        ...
        if ($response['process'] == 'complete' && $demo == 'shortcodes') {
            ...
            $shortcodes_page = porto_importer_get_page_by_title( 'Shortcodes' );
...
function porto_importer_get_page_by_title( $page_title, $output = OBJECT ) {
    global $wpdb;
    $sql = $wpdb->prepare( "
            SELECT ID
            FROM $wpdb->posts
