...
jQuery(function($) {
    ...
    function addAlertLeavePage() {
        $('#porto-import-yes').attr('disabled', 'disabled');
        $(window).bind('beforeunload', alertLeavePage);
    }

    function removeAlertLeavePage() {
        $('#porto-import-yes').removeAttr('disabled');
    ...
    function porto_import_dummy(options) {
        ...
        if (options.import_dummy) {
            var demo = options.demo,
                data = {'action': 'porto_import_dummy', 'process':'import_start', 'demo': demo, 'override_contents': options.override_contents};
    ...
    function porto_import_finished(options) {
        ...
        setTimeout(function() {
            ...
            jQuery('.porto-install-demo .porto-demo-install').html(jQuery("#porto-install-options .theme-name").text() + ' installation is finished!');
    ...
    jQuery('body').on('click', '.button-load-plugins', function(e) {
        e.preventDefault();
        jQuery(this).closest('.feature-section').children('.theme.hidden').hide();
        jQuery(this).closest('.feature-section').children('.theme.hidden').fadeIn();
        jQuery(this).closest('.feature-section').children('.theme.hidden').removeClass('hidden');
    });
    ...
    jQuery(document).on('click', '.porto-install-demos .theme .theme-wrapper', function(e) {
        ...
        if (jQuery(this).closest('.theme').hasClass('open-classic')) {
            ...
            if (typeof jQuery('#porto-import-yes').attr('disabled') == "undefined") {
