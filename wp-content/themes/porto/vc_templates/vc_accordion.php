...
$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'accordion '.$el_class.' '.$type.($type == 'custom' && $skin != 'custom' ? ' accordion-' . $skin . ' ' : '') . ' ' . $size, $this->settings['base']);

if ($type == 'custom' && $skin == 'custom' && ($heading_color || $heading_bg_color)) {
    $output .= '<style type="text/css">';
    if ($heading_color) $output .= '#' . $id . '.accordion .card-header a { color: ' . $heading_color . ' }';
    if ($heading_bg_color) $output .= '#' . $id . '.accordion .card-header { background-color: ' . $heading_bg_color . ' }';
    $output .= '</style>';
}
